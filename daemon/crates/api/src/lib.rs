...
use daemon8_types::{Checkpoint, Filter, Observation, ObservationKindTag, OriginPattern, Severity};
...
// Mirrors the equivalent in daemon8-mcp; keep in sync if the pattern syntax changes.
...
fn parse_filter(
    ...
    let kinds = kinds.map(|raw| {
        raw.split(',')
            .filter_map(|s| {
                let s = s.trim();
                match s.parse::<ObservationKindTag>() {
                    Ok(k) => Some(k),
                    Err(e) => {
    ...
    let severity_min = severity_min.and_then(|raw| match raw.trim().parse::<Severity>() {
        Ok(sev) => Some(sev),
        Err(e) => {
            warn!("ignoring unknown severity '{}': {}", raw, e);
            None
        }
    });

    let origins = origins.map(|raw| {
        raw.split(',')
            .map(|s| parse_origin_pattern(s.trim()))
            .collect::<Vec<_>>()
    });
...
async fn handle_observe(
    ...
    let filter = parse_filter(
        ...
        params.text_match,
        params.since,
        Some(params.limit.unwrap_or(50).min(500) as usize),
    );
...
async fn handle_connections(State(state): State<ApiState>) -> Response {
    match state.store.summary() {
        Ok(summary) => {
            let body = serde_json::json!({ "connections": summary.connections });
            (StatusCode::OK, Json(body)).into_response()
        }
        Err(e) => error_json(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("connections query failed: {e}"),
        ),
    }
}
...
async fn handle_stream(
    ...
    tokio::spawn(async move {
        ...
        if let Some(since_id) = last_event_id {
            ...
            match store.query(&replay_filter) {
    ...
    match req {
        ...
        ActRequest::InjectCss {
            ...
            dispatch_action(
                ...
                |element_id| ok_json(serde_json::json!({ "element_id": element_id })),
            )
            .await
        }
        ActRequest::RevertCss { tab_id } => {
            dispatch_action(
                ...
                |count| ok_json(serde_json::json!({ "reverted": count })),
            )
            .await
        }
        ...
        ActRequest::Navigate { tab_id, url } => {
            dispatch_action(
                ...
                |final_url| ok_json(serde_json::json!({ "navigated": true, "url": final_url })),
            )
            .await
        }
