...
fn default_udp_bind() -> SocketAddr {
    SocketAddr::new(IpAddr::from([127, 0, 0, 1]), 9078)
}
...
pub fn resolve_db_path(config_path: Option<&std::path::Path>) -> PathBuf {
    ...
    project_dirs()
        .map(|d| d.data_dir().join("observations.db"))
        .unwrap_or_else(|| PathBuf::from("observations.db"))
}
...
mod tests {
    ...
    fn resolve_db_path_default() {
        ...
        assert!(p.to_string_lossy().contains("observations.db"));
    }
    ...
    fn resolve_db_path_default_uses_profile_tagged_slug() {
        ...
        assert!(s.contains("observations.db"));
    }

    #[test]
    fn default_config_is_sane() {
        let cfg = Config::default();
        assert_eq!(cfg.server.port, 9077);
...
"#,
        ...
        .unwrap();
        assert_eq!(cfg.ingestion.udp.bind.port(), 9078);
