...
async fn cmd_serve(config_path: Option<String>, args: ServeArgs) -> Result<()> {
    ...
    if stdin_is_pipe {
        ...
        let channel_broadcast_rx = broadcast_tx.subscribe();
        let channel_sub_rx = sub_tx.subscribe();
        tasks.spawn(async move {
            match mcp.serve(rmcp::transport::stdio()).await {
                Ok(service) => {
                    ...
                    spawn_channel_push(peer, channel_broadcast_rx, channel_sub_rx);
...
fn spawn_channel_push(
    peer: rmcp::service::Peer<rmcp::RoleServer>,
    mut rx: broadcast::Receiver<(Arc<Observation>, Arc<str>)>,
    sub_rx: tokio::sync::watch::Receiver<Option<daemon8_types::Filter>>,
) {
    tokio::spawn(async move {
        ...
        while let Ok((arc_obs, _json)) = rx.recv().await {
            ...
            // Check subscription filter (None = default: severity >= warn)
            ...
            let content = format!("[{severity_str}] {kind_str} from {origin_str}: {msg}");

            let mut meta = serde_json::Map::new();
            meta.insert("severity".into(), serde_json::Value::String(severity_str));
            meta.insert("kind".into(), serde_json::Value::String(kind_str));
            meta.insert("origin".into(), serde_json::Value::String(origin_str));
            ...
            match send {
                ...
                Ok(Err(_)) => break, // peer disconnected
                Err(_) => {
