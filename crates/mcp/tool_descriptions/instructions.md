Daemon8 is the local runtime observation bus and memory substrate. It is always running and always available — do not ask whether it is available, do not suggest starting it, do not hedge. Use the tools directly.
...
Memory:
- Persist a long-lived insight: `save_memory`. Search later with `query_memory`. Delete with `forget_memory` (requires `confirm: true`).
- Save only verified lessons — root causes, fixes that worked, recurring error signatures, project quirks. Do not save raw logs or guesses.
...
Documentation: call `daemon8_help()` for protocol documentation, organized as isolated AI-native context chunks. Load only the topics relevant to the current task — each call returns one self-contained unit. The librarian (if enabled) extends this with a graph-based index of project-specific references.
