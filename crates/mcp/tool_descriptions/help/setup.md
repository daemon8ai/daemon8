...
2. Open the generated config. The markdown body contains step-by-step instructions: verify the auto-detected stack, audit all log paths, and populate the `sources` array.
...
4. Explain the sources to the user in plain terms: "these are the runtime signals daemon8 watches so it can surface what is happening in real time." Present the updated config and confirm.
5. Retry `daemon8_connect`. If the response includes a `requirements` field, complete every listed action before proceeding.
