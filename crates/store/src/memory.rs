...
impl StateModel for MemoryStore {
    ...
    fn query(&self, filter: &Filter) -> Result<StateSlice, StoreError> {
        ...
        if let Some(ref cid) = filter.correlation_id {
            results.retain(
                |o| matches!(&o.correlation_id, Some(obs_cid) if obs_cid.as_ref() == cid.as_str()),
            );
        }
        ...
        {
            results.retain(|o| match &o.tags {
                Some(obs_tags) => required_tags.iter().all(|t| obs_tags.contains(t)),
                None => false,
            });
        }

        // limit: take the newest N (from the end), keep chronological order
