...
use crate::{MemoryModel, StateModel, StoreError, semantic};
...
impl SqliteStore {
    ...
    pub fn open(path: &Path) -> Result<Self, StoreError> {
        ...
        // v0.1.1 schema migration: add correlation/graph/identity columns.
        let has_correlation: bool = conn
            .prepare("SELECT correlation_id FROM observations LIMIT 0")
            .is_ok();
        if !has_correlation {
            conn.execute_batch(
                "ALTER TABLE observations ADD COLUMN correlation_id TEXT;
                 ...
                 ALTER TABLE observations ADD COLUMN node_id TEXT;
                 CREATE INDEX IF NOT EXISTS idx_obs_correlation ON observations(correlation_id);
                 CREATE INDEX IF NOT EXISTS idx_obs_session ON observations(session_id);",
            )?;
            tracing::info!(
                "migrated schema: added correlation_id, parent_id, tags, session_id, node_id columns"
            );
        }
...
impl StateModel for SqliteStore {
    fn insert(&self, obs: Observation) -> Result<u64, StoreError> {
        ...
        let correlation_id = obs.correlation_id.as_deref();
        let tags_json = obs
            .tags
            .as_ref()
            .map(|t| serde_json::to_string(t).unwrap_or_default());
        let parent_id = obs.parent_id.map(|id| id as i64);
        let session_id = obs.session_id.as_deref();
        let node_id = obs.node_id.as_deref();

        guard.conn.execute(
            "INSERT INTO observations (timestamp_ns, severity, kind_tag, origin_json, kind_json, data, source_file, source_line, correlation_id, parent_id, tags, session_id, node_id)
        ...
        let inserted_id = guard.conn.last_insert_rowid() as u64;
        let memory_text =
            semantic::observation_text(&origin_json, &kind_tag_str, &data_str, source_file);
        let memory_metadata = serde_json::json!({
            "severity": severity_str,
            "kind_tag": kind_tag_str,
            "origin": obs.origin,
    ...
    fn query(&self, filter: &Filter) -> Result<StateSlice, StoreError> {
        ...
        if let Some(ref cid) = filter.correlation_id {
            clauses.push(format!("correlation_id = ?{}", param_values.len() + 1));
            param_values.push(Box::new(cid.clone()));
        }

        if let Some(ref required_tags) = filter.tags {
            for tag in required_tags {
                clauses.push(format!("tags LIKE ?{}", param_values.len() + 1));
        ...
        let sql = format!(
            "SELECT id, timestamp_ns, severity, kind_tag, origin_json, kind_json, data, source_file, source_line, correlation_id, parent_id, tags, session_id, node_id
             FROM observations {where_clause} ORDER BY id DESC {limit_clause}"
        );
        ...
        let rows = stmt.query_map(params_ref.as_slice(), |row| {
            Ok(RawRow {
                ...
                correlation_id: row.get::<_, Option<String>>(9)?,
                parent_id: row.get::<_, Option<i64>>(10)?,
                tags: row.get::<_, Option<String>>(11)?,
                session_id: row.get::<_, Option<String>>(12)?,
                node_id: row.get::<_, Option<String>>(13)?,
            })
        })?;
        ...
        for row_result in rows {
            ...
            let tags: Option<Vec<String>> = raw
                .tags
                .as_deref()
            ...
            observations.push(Observation {
                ...
                source_location,
                timestamp_ns,
                correlation_id: raw.correlation_id.map(Arc::from),
                parent_id: raw.parent_id.map(|id| id as u64),
                tags,
                session_id: raw.session_id.map(Arc::from),
                node_id: raw.node_id.map(Arc::from),
            });
        }
...
impl MemoryModel for SqliteStore {
    fn index_document(
        &self,
        source_kind: &str,
        source_key: &str,
    ...
    fn search_memory(
        ...
        query: &str,
        limit: usize,
    ) -> Result<Vec<crate::MemoryHit>, StoreError> {
        let guard = self.inner.lock().map_err(|_| StoreError::LockPoisoned)?;
        semantic::search(&guard.conn, query, limit)
    }
}
