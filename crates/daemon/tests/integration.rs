...
// ---------------------------------------------------------------------------
// Specialist lifecycle HTTP routes
//
// The handlers in `crates/api/src/lib.rs` (`handle_specialist_{start,stop,
// restart,patch}`) own only auth/parsing/error-mapping. The interesting
// behavior lives in the controller they delegate to. We use a mock
// `SpecialistController` here so we can drive every status-code branch in
// `map_specialist_error` without standing up SurrealKV + a real registry.
// ---------------------------------------------------------------------------

mod specialist_routes {
    use super::*;
    ...
    type StartFn = Box<dyn Fn(&str) -> Result<(), SpecialistControlError> + Send + Sync>;
    type PatchFn = Box<dyn Fn(&str) -> Result<bool, SpecialistControlError> + Send + Sync>;

    /// Test double: every method returns a canned `Result` regardless of
    /// the slug. The tests pick which canned outcome they need.
    ...
    impl CannedController {
        fn ok() -> Arc<dyn SpecialistController> {
            Arc::new(Self {
                start: Box::new(|_| Ok(())),
                stop: Box::new(|_| Ok(())),
                restart: Box::new(|_| Ok(())),
                patch: Box::new(|_| Ok(true)),
            })
        }

        fn always<F>(error_fn: F) -> Arc<dyn SpecialistController>
        where
            F: Fn() -> SpecialistControlError + Send + Sync + 'static + Clone,
        {
            let f1 = error_fn.clone();
            let f2 = error_fn.clone();
            let f3 = error_fn.clone();
            let f4 = error_fn;
            Arc::new(Self {
                start: Box::new(move |_| Err(f1())),
                stop: Box::new(move |_| Err(f2())),
                restart: Box::new(move |_| Err(f3())),
                patch: Box::new(move |_| Err(f4())),
            })
        }
    }

    #[async_trait]
    impl SpecialistController for CannedController {
    ...
    async fn start_with_controller(
        controller: Option<Arc<dyn SpecialistController>>,
    ) -> (String, tokio::task::JoinHandle<()>) {
        let (chrome_cmd_tx, _rx) = mpsc::channel::<ChromeCommand>(16);
        let (stream_tx, _) = broadcast::channel::<StreamFrame>(16);
        let store: Arc<dyn StateModel> = Arc::new(SurrealStore::memory().await.unwrap());
        let (_, chrome_state_rx) =
            tokio::sync::watch::channel(daemon8_chrome::ConnectionState::Connected);
        ...
        let api_state = daemon8_api::ApiState {
            ...
            bookkeeper_store: None,
            embedding_profile_store: None,
            specialist_controller: controller,
        };
        let app = daemon8_api::api_router(api_state);
    ...
    async fn post(base: &str, path: &str) -> reqwest::StatusCode {
        reqwest::Client::new()
    ...
    async fn patch_json(base: &str, path: &str, body: Value) -> reqwest::StatusCode {
        reqwest::Client::new()
    ...
    #[tokio::test]
    async fn start_returns_200_when_controller_succeeds() {
        let (base, _h) = start_with_controller(Some(CannedController::ok())).await;
        let s = post(&base, "/api/deliber8/agents/alpha/start").await;
        assert_eq!(s, 200);
    }

    #[tokio::test]
    async fn start_returns_503_when_controller_is_none() {
        let (base, _h) = start_with_controller(None).await;
        let s = post(&base, "/api/deliber8/agents/alpha/start").await;
        assert_eq!(s, reqwest::StatusCode::SERVICE_UNAVAILABLE);
    }

    #[tokio::test]
    async fn start_returns_404_on_not_found() {
        let ctrl = CannedController::always(|| SpecialistControlError::NotFound {
            slug: "missing".into(),
        });
        let (base, _h) = start_with_controller(Some(ctrl)).await;
        let s = post(&base, "/api/deliber8/agents/missing/start").await;
        assert_eq!(s, reqwest::StatusCode::NOT_FOUND);
    }

    #[tokio::test]
    async fn start_returns_424_on_missing_api_key() {
        let ctrl = CannedController::always(|| SpecialistControlError::MissingApiKey {
            var: "OPENROUTER_API_KEY".into(),
        });
        let (base, _h) = start_with_controller(Some(ctrl)).await;
        let s = post(&base, "/api/deliber8/agents/x/start").await;
        assert_eq!(s, reqwest::StatusCode::FAILED_DEPENDENCY);
    }

    #[tokio::test]
    async fn start_returns_409_on_already_running() {
        let ctrl = CannedController::always(|| SpecialistControlError::AlreadyRunning {
            slug: "x".into(),
        });
        let (base, _h) = start_with_controller(Some(ctrl)).await;
        let s = post(&base, "/api/deliber8/agents/x/start").await;
        assert_eq!(s, reqwest::StatusCode::CONFLICT);
    }

    #[tokio::test]
    async fn stop_returns_409_on_not_running() {
        let ctrl =
            CannedController::always(|| SpecialistControlError::NotRunning { slug: "x".into() });
        let (base, _h) = start_with_controller(Some(ctrl)).await;
        let s = post(&base, "/api/deliber8/agents/x/stop").await;
        assert_eq!(s, reqwest::StatusCode::CONFLICT);
    }

    #[tokio::test]
    async fn restart_returns_400_on_bad_config() {
        let ctrl = CannedController::always(|| SpecialistControlError::BadConfig {
            slug: "x".into(),
            reason: "no provider".into(),
        });
        let (base, _h) = start_with_controller(Some(ctrl)).await;
        let s = post(&base, "/api/deliber8/agents/x/restart").await;
        assert_eq!(s, reqwest::StatusCode::BAD_REQUEST);
    }

    #[tokio::test]
    async fn patch_with_empty_body_returns_400() {
        let (base, _h) = start_with_controller(Some(CannedController::ok())).await;
        // Both persona and model omitted → handler-level guard fires.
        let s = patch_json(&base, "/api/deliber8/roster/alpha", json!({})).await;
        assert_eq!(s, reqwest::StatusCode::BAD_REQUEST);
    }

    #[tokio::test]
    async fn patch_with_persona_only_returns_200() {
        let (base, _h) = start_with_controller(Some(CannedController::ok())).await;
        let s = patch_json(
            &base,
            "/api/deliber8/roster/alpha",
            json!({"persona": {"identity_prompt": "x"}}),
        )
        .await;
        assert_eq!(s, 200);
    }
