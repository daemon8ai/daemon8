...
pub fn cmd_uninstall() -> Result<()> {
    ...
    // 4. Project-local .daemon8-cli.toml
    ...
    let home = crate::provider::dirs_home();
...
fn remove_provider_entry(provider: Provider, home: &std::path::Path) {
    ...
    if provider == Provider::ClaudeCode {
        let ok = crate::provider::shim_command("claude")
        ...
        let _ = crate::provider::shim_command("claude")
    ...
    if provider == Provider::Gemini {
        let ok = crate::provider::shim_command("gemini")
