...
pub async fn run_agent(args: AgentArgs) -> Result<()> {
    ...
    if args.background {
        run_background(args, &daemon_url, &project_root).await
    } else {
        run_one_shot(args, &daemon_url, &project_root).await
    }
}
...
async fn run_background(
    ...
    loop {
        flush_due_prompts(&args, daemon_url, project_root, &model, &mut pending).await?;
...
async fn handle_stream_event(
    ...
    flush_due_prompts(args, daemon_url, project_root, model, pending).await
}
